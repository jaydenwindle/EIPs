...
The singleton registry also supports ephemeral wallets which are created and then destroyed within the same transaction by using `selfdestruct` after executing a call. This design is already utilized in several existing DeFi projects.

This feature can be implemented entirely at the wallet level by have it call `selfdestruct` on itself at the end of its `executeCall` function. This allows implementations which support ephemerality to utilize the same registry contract as those that don't for additional optional security. Additional registry contracts can be used to reduce any addititonal complexity placed on the user.
