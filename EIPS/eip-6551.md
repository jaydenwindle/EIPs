...
author: Jayden Windle (@jaydenwindle), Benny Giang <bg@futureprimitive.xyz>, Steve Jang, Druzy Downs (@druzydowns), Raymond Huynh (@huynhr), Alanah Lam <alanah@futureprimitive.xyz>, Josh Weintraub (@jhweintraub) <josh@revest.finance>, Rob Montgomery (@RobAnon) <rob@revest.finance>
...
Since the EIP specifies that wallets are to be created using a deterministic `CREATE2` factory, this allows for optional use of ephemeral wallets by the controller. Ephemeral wallets are ones which are created and then destroyed within the same transaction. By selfdestructing the wallet at the end of the transaction this introduces new security features. Since no code exists between transactions, there is no way for a malicious user to exfiltrate any funds, and only the holder of a specified NFT is capable of invoking the `CREATE2` opcode to redeploy this wallet. By having the destination of the `selfdestruct` be `payable(address(this))`, users can still store whatever native-ETH they want until transferred out. An attacker would need to identify vulnerabilities in both the registry and the wallet itself to deploy and steal any stored funds. This design is already utilized in several existing DeFi projects such as Revest Finance, Pine Finance, and JPEGd.

This feature can be implemented entirely at the wallet level. The wallet would call `selfdestruct` on itself at the end of its `executeCall` function, with the code being erased at the conclusion of the transaction. Since this function can only be invoked by a proper owner, the ability to selfdestruct does not prevent a security risk. This allows implementations which support ephemerality to utilize the same registry contract as those that don't, reducing additional complexity. A user which uses ephemeral wallets simply has to call `createAccount` to redeploy before using their wallet. It is possible to use a different registry contract to reduce this to a single-call, but is left as an exercise for the reader. 

```
contract ExampleERC6551Account {
    function executeCall(
        address to,
        uint256 value,
        bytes calldata data
...
Any potential downsides to ephemeral wallets can be alleviated by simply moving certain on-chain actions off-chain. First, while the code would not be accessible between transactions to view information such as token address and ID, this information should already be known to users, and the address of the contract can still be calculated at any time using the `account` function to view stored-funds. Second, while the lack of code means that the `isValidSignature` function cannot be called directly on-chain between transactions, this can always be performed off-chain. Since the owner of the wallet is just the owner of the specified NFT, a signature can be checked against their address off-chain as well, without requiring the code exist on-chain.
